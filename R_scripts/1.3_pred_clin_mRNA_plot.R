...
clin_available_mRNA <- data.frame(matrix(ncol = 6, nrow = length(cancers_used)))
colnames(clin_available_mRNA) <- c("age", "gender", "grade", "T_stage", "N_stage", "M_stage")
row.names(clin_available_mRNA) <- cancers_used

# vector for the plot
clin_text_plot <- rep(NA, length(cancers_used))
names(clin_text_plot) <- cancers_used
...
p_val_vect_C = p_val_vect_IBS  <- rep(NA, length(cancers_used))
names(p_val_vect_C) = names(p_val_vect_IBS)  <- cancers_used
